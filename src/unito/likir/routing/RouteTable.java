...
public interface RouteTable
{
    ...
    public static enum PurgeMode
    {
        ...
        DROP_CACHE,
        /**
         * Deletes all non-alive Contacts from the active RouteTable and fill up
         * the new slots with alive Contacts from the replacement cache
         */
        PURGE_CONTACTS,
        /**
         * Merges all Buckets by essentially rebuilding the entire RouteTable
         */
        MERGE_BUCKETS,
        /**
    ...
    /**
     * Returns 'count' number of Contacts that are nearest (XOR distance) to the
     * given NodeId.
     */
    ...
    /**
     * This method tries to replace an existing Contact in the given Bucket with
     * the given Contact or tries to add the given Contact to the Bucket's
     * replacement Cache. There are certain conditions in which cases we replace
     * Contacts and if it's not possible we're trying to add the Contact to the
     * replacement cache.
     */
    ...
    /**
     * This method splits the given Bucket into two new Buckets. There are three
     * conditions for splitting: 1. Bucket contains the local Node 2. New node
     * part of the smallest subtree to the local node 3. current_depth mod
     * symbol_size != 0 //SPIEGA MEGLIO
     */
