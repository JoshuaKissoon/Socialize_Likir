 ...
 * The StorageCleaner is executed periodically for a good DHT functioning. A
 * StorageCleaner instance is always related to the local Node instance.
 *
...
public class StorageCleaner implements Runnable
{
    ...
    {
        System.out.println(" ************** Running Storage Maintenance ****************** ");
            ...
            for (StorageEntry entry : storage.values())
            {
                ...
                else if (currentTime - entry.getLastRepublishTime() > CONTENT_REPUBLISH_PERIOD)
                {
                    ...
                    for (Contact c : nearestNodes)
                    {
                        ...
                        {
                            node.store(c, entry);
                            System.out.println("Stored " + new String(entry.getContent().getValue()) + " at " + c.getNodeId());
                        }
        ...
        System.out.println(" New Storage: " + node.getStorage().toString());
        System.out.println(" ************** Running Storage Maintenance Completed ****************** ");
